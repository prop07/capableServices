...
const Page = ({ params }: { params: { slug: string } }) => {
    const blog = blogData.find((blog) => blog.id === parseInt(params.slug));

    if (!blog) {
        return (
            <div className='py-32 text-read text-center'>
                <p>
                    Blog not found. It may have been removed or does not exist.
                </p>
                <p><Link href={"/blog"}><span className='text-blue-500 underline font-semibold'>blog</span></Link></p>
            </div>
        )
    }
    ...
    return (
        <div className='md:pt-16 pt-24 py-4 md:w-[70vw]  justify-center mx-auto'>
            <div className={`w-full  mx-auto mb-4 p-2 `}>
                ...
                <div className='mt-2 space-y-1'>
                    <div className=' text-read' dangerouslySetInnerHTML={{ __html: blog.content }} />
                </div>
            </div>
        </div>
    )
